...
public class ManualReload : CustomInteractible
{
    ...
    //[HideInInspector]
	public bool reloadHalf,reloadEnd=true,reloadFinish=true,handDrop,boltAngleTrue=false,boltSlideTrue = true,holdOpen = false; //reload maintaince variables
	...
	PrimitiveWeapon primitiveWeapon;
	Trigger trigger;
...
//	[HideInInspector]
	...
	void FixedUpdate()
    {
		if (typeReload == TypeReload.Slider&&returnAddSpeed > 0 || knockback > 0) {		
			if ((reloadHalf||handDrop)&& !holdOpen) {
